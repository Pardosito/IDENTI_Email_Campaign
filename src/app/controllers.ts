...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const match = await bcrypt.compare(password, (foundUser as IUser).password);
    if (!match) {
...
export const createContact = async (req: Request, res: Response) => {
  try {
    ...
    const doesContactExist = await Contact.findOne({ contactEmail });
...
export const updateContact = async (req: Request, res: Response) => {
  try {
    ...
    if (contactEmail && contactEmail !== (contactToUpdate as IContact).email) {
...
export const getCampaignPage = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).send(`Error loading campaign page: ${error}`);
  }
};
